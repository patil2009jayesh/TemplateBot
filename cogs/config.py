...
class ConfigCog(commands.Cog, name="config"):
    ...
    async def settings_cmd(self, interaction: discord.Interaction):
        ...
        async with get_db() as db:
            ...
            async with db.execute("SELECT COUNT(*) FROM warnings WHERE guild_id = ?", (str(guild.id),)) as wc:
                warning_count = (await wc.fetchone())[0]

        embed = discord.Embed(
            title=f"🛠️ Server Setup Audit — {guild.name}",
            description=(
                f"Complete diagnostic of every feature, channel, role & filter.\n"
                f"Bot: 🟢 **Online** • `{guild.member_count}` members"
            ),
        ...
        # ── 1. Auto-VC ──
        # /setupvc stores: settings["autovc"]["trigger_channel_id"]
        autovc_cfg = settings.get("autovc", {})
        autovc_trigger_id  = autovc_cfg.get("trigger_channel_id")
        autovc_category_id = autovc_cfg.get("category_id")
        try:
            autovc_trigger_ch  = guild.get_channel(int(autovc_trigger_id))  if autovc_trigger_id  else None
            autovc_category_ch = guild.get_channel(int(autovc_category_id)) if autovc_category_id else None
        except (ValueError, TypeError):
            autovc_trigger_ch = autovc_category_ch = None

        autovc_status = (
            f"• **Module:** {mod_badge('auto_vc')}\n"
            f"• **Trigger Hub:** {'✅ ' + autovc_trigger_ch.mention + f' (`{autovc_trigger_ch.id}`)' if autovc_trigger_ch else '⚠️ *Not Configured — run `/setupvc trigger_channel:<voice>`*'}\n"
            f"• **Category:** {'✅ ' + autovc_category_ch.name if autovc_category_ch else '`Default (same as trigger)`'}\n"
        ...
        welcome_status = (
            ...
            f"• **Welcome System:** `{'🟢 Enabled' if welcome_enabled else '🔴 Disabled'}` (`/setwelcome`)\n"
            ...
            f"• **Auto-Role:**       {role_repr(roles.get('autorole'), '/setautorole role:<role>')}\n"
            f"• **Template:** *\"{welcome_msg[:80]}{'...' if len(welcome_msg) > 80 else ''}\"*"
        )
        embed.add_field(name="👋 2. Welcome & Onboarding", value=welcome_status, inline=False)
        ...
        automod_status = (
            ...
            f"**Max Mentions:** `{automod.get('mention_limit', 0)} pings`\n"
            f"• **Total Warnings Logged:** `{warning_count}`"
        )
        embed.add_field(name="🛡️ 3. AutoMod & Server Logs", value=automod_status, inline=False)

        # ── 4. Leveling ──
        # /levelreward stores in settings["leveling"]["rewards"]
        rewards_dict = level_cfg.get("rewards", {}) or settings.get("level_rewards", {})
        rewards_formatted = []
        if rewards_dict:
            for lvl, r_id in sorted(rewards_dict.items(), key=lambda x: int(x[0])):
                try:
                    role_obj = guild.get_role(int(r_id))
                except (ValueError, TypeError):
                    role_obj = None
                rewards_formatted.append(
                    f"Level `{lvl}` ➡️ {role_obj.mention if role_obj else f'Deleted Role (`{r_id}`)'}"
                )
        rewards_text = ", ".join(rewards_formatted) if rewards_formatted else "⚠️ *None configured (`/levelreward`)*"

        level_ch_id = channels.get("leveling") or channels.get("level_ups")
        level_status = (
            f"• **Module:** {mod_badge('leveling')}\n"
            f"• **XP Multiplier:** `{level_cfg.get('xp_multiplier', 1.0)}x` • "
            f"**Range:** `{level_cfg.get('min_xp', 15)}–{level_cfg.get('max_xp', 25)} XP`\n"
            f"• **Cooldown:** `{level_cfg.get('cooldown', 60)}s` between XP gains\n"
            f"• **Level-Up Channel:** {ch_repr(level_ch_id, '/setchannel type:leveling') if level_ch_id else '`Default: Same channel as message`'}\n"
        ...
        if raw_teams:
            for t in raw_teams:
                try:
                    t_id, t_name, s_role_id, cat_id = t["id"], t["team_name"], t["staff_role_id"], t["category_id"]
                except Exception:
                ...
                teams_formatted.append(
                    f"• **#{t_id} {t_name}**: Staff {s_role.mention if s_role else '⚠️ None'} | Cat: `{cat.name if cat else '⚠️ None'}`"
                )
            teams_text = "\n".join(teams_formatted)
        else:
            teams_text = "⚠️ *No teams created yet — run `/ticket setup-team`*"

        ticket_status = (
            f"• **Module:** {mod_badge('tickets')}\n"
            f"• **Teams:**\n{teams_text}\n"
            f"• **Open Tickets:** `{open_tickets}` currently active"
        )
        embed.add_field(name="🎫 5. Ticket System", value=ticket_status, inline=False)

        # ── 6. Roles, Giveaways & Commands ──
        cmd_preview = ", ".join(f"`!{c}`" for c in custom_cmds[:8]) if custom_cmds else "*None (`/addcmd`)*"
        extra_status = (
            f"• **Button Roles:** `{btn_roles_count}` panels (`/buttonrole`)\n"
            f"• **Reaction Roles:** `{rxn_roles_count}` binds (`/reactionrole`)\n"
            f"• **Active Giveaways:** `{active_gw}` running (`/giveaway`)\n"
            f"• **Custom Commands:** {cmd_preview}"
        )
        embed.add_field(name="🎁 6. Self-Roles, Giveaways & Commands", value=extra_status, inline=False)

        embed.set_footer(
            text="Tachos Dev • Run any unconfigured command to complete setup",
